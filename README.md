...
- DATASUS: `scan -> inventory -> dataset families -> family registry with member files -> family-specific candidate selection -> selective download -> local representative profiling -> variable catalog / similarity artifacts -> translation bundle -> validate/merge/coverage`
...
- profile only a tiny curated candidate set per family
